...
public class NetworkPlayer : MonoBehaviour {
	...
	void RecieveData(ushort senderID, byte tag, ushort subject, object data){
		...
		if (senderID == networkID) {
			...
			if (tag == TagIndex.PlayerMessage)
			{
				if (subject == TagIndex.PlayerMessageSubjects.Emotion)
				{
					Debug.Log("Receving Player Emotion :" + (string) data, gameObject);
				}
				else if(subject == TagIndex.PlayerMessageSubjects.Text)
				{
					Debug.Log("Received player text message: " + (string) data, gameObject);
				}
			}
